	...
	cat >&2 <<-'EOW'
		...
		📎 Hey there!  It looks like you're trying to run a balenaEngine daemon.
		   ...
		   You probably should use the "beind" image variant instead, something like:
		     ...
		     docker run --privileged --name some-overlay-balena -d balena/engine:beind --storage-driver=overlay

		   See https://hub.docker.com/balena/engine/ for more documentation and usage examples.
